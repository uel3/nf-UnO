...
The salmonella outbreak set of data consists of 10 sets of paired end (PE) reads. These reads should be passed to the nf-UnO using a samplesheet.csv with 4 required columns, the 5th column is not required for this version of the nf-UnO pipeline. Future versions will include added support for long reads. 
...
```
## MultiQC output 
In the docs folder [https://github.com/uel3/nf-UnO/tree/main/docs] there is a MultiQC Report for nf-uno analysis of the salmonella outbreak data set. 
...
![General Stats](https://github.com/uel3/nf-UnO/blob/main/images/general_stats.svg)
...
![MIDAS2](https://github.com/uel3/nf-UnO/blob/main/images/NFUNOmidas2.svg)

### FastQC: Raw Reads
**spot for metrics to highlight/consider when evaluating how seqeuncing went**

![FastQC: Raw Reads](https://github.com/uel3/nf-UnO/blob/main/images/FastqcRaw.svg)

### Bowtie2: Host Removal 
Bowtie2 mapping statistics of input reads mapped to host for removal using GRCh38 reference genome. The user can view the mapping statistics as number of reads or percentage of total reads. The portion of mNGS reads in red are the only reads used in downstream processes, with human host sequences removed. mNGS reads that are blue, light blue, orange, or yellow are considered host reads are discarded, and not included in downstream analysis. In the example below, you can see that only sample SRR5058924 has a substantial number of reads aligned to the GRCh38 reference genome that are removed by this process. 

![Host Removal](https://github.com/uel3/nf-UnO/blob/main/images/BT2HostRemoval.svg)
...
![FastQC: After Preprocessing](https://github.com/uel3/nf-UnO/blob/main/images/FastqcPostPreProcess.svg)

### Binning: Sample Coverage of MaxBin2, MetaBAT2, and DASTool Bins
Sample reads are mapped against DASTool refined bins to calculate bin depths with log transformation. 

Only bins with reads mapped from all samples are of interest for downstream analysis. nf-UnO can output DASTool refined bins only (seen in the example below) or both refined DASTool bins and unrefined bins. If the user is interested in unrefined bins from the MaxBin2 and MetaBat2 binners along with DASTool refined bins, adding ```--postbinning_input 'both'``` to CLI pipeline execution will generate a heatmap for all 3 binning programs. 
...
![Binning](https://github.com/uel3/nf-UnO/blob/main/images/Binning.svg)

### CheckM: Bin Quality Statistics
Qaulity assessement of genome bins using CheckM.

The user can see the quality metrics of the species bins generated from all outbreak reads. The user can interpret the quality of the species bins based on combinations of the following metrics: completeness, contamination, and strain heterogeneity. Genome size can also be indicative of species bin quality. Moderate to high quality bins will have >50% completeness and <10% contamination and are identified by green highlighting. Low quality bins will have completeness <50% and contamination <10% and are identified by yellow highlighting. Strain heterogeneity should be interpreted as the amount of contamination attributed to closely related species. *Note the binning process is prone to binning closely related species together which can lead to contamination, but this type of contamination is not in any way related to mishandling in lab (ie contaminated specimens).*

![CheckM](https://github.com/uel3/nf-UnO/blob/main/images/checkm.svg)

### GTDB-tk Taxonomy of Bins
Taxonomy assignemnt of bins

The user can assign taxonomy to all bins generated by the DASTool, MaxBin2, and MetaBAT2 using the Genome Taxonomy Database. The file path to download the GTDB-tk databse is provided in the nextflow.config file but the user can specify a local copy of the database to save time and space. The database is quite large (110GB) and will take time to download. We recommend downloading a local copy prior to running the pipeline and supply it to to the gtdb_db parameter in the nextflow.config or ```--gtdb_db``` from the command line. 

![GTDB-tk](https://github.com/uel3/nf-UnO/blob/main/images/gtdbtk.svg)
